...
module CP
	...
	module Object
		...
		def draw(window)
		if @chipmunk_objects	## if this is a composite object, call draw() on primitives
	...
	include GL, GLU, GLUT
		...
		def draw_segment_shape(seg)
			...
			if (seg.radius != 0)
				...
				var = PillVAR.dup
				(0...PillVAR_count).step(2) do |i| {var[i] += len}
